...
import { CoachingWaitlistCta } from "@/components/waitlist/CoachingWaitlistCta";
...
import { isFourWeekSupportPage, isPersonalDietReviewPage, isSupportOfferPage, type FourWeekSupportContent, type PersonalDietReviewContent, type SupportOfferCard } from "@/lib/support-offer-copy";
...
function Hero({ page, preview }: { page: PrivatePage; preview: boolean }) {
  ...
  const personalDietReview = isPersonalDietReviewPage(page) ? page.content : null;
...
function FourWeekSupportDetail({ page, content }: { page: PrivatePage; content: FourWeekSupportContent }) {
  const ctaUrl = content.cta.active ? page.salesLinks[content.cta.salesLinkKey] : undefined;
  return <div className="overflow-hidden bg-white"><Container className="py-[var(--space-section)]"><div className="mx-auto max-w-4xl space-y-8"><ContentSection title={content.audienceTitle}><BulletList items={content.audience} /></ContentSection><ContentSection title={content.benefitsTitle}><p className="mb-6 text-lg leading-relaxed text-[var(--color-text-muted)]">{content.benefitsIntro}</p><DetailList items={content.benefits} /></ContentSection><ContentSection title={content.processTitle}><DetailList items={content.process} numbered /></ContentSection><FourWeekSupportPurchase content={content} ctaUrl={ctaUrl} /><FourWeekEverydayLife content={content} /><FourWeekSupportFaq items={content.faq} /><FourWeekSupportFinalCta content={content} ctaUrl={ctaUrl} />{content.additionalInfo ? <p className="rounded-2xl bg-[var(--color-surface-muted)] p-6 text-sm leading-relaxed text-[var(--color-text-muted)]">{content.additionalInfo}</p> : null}</div></Container></div>;
}

function FourWeekSupportPurchase({ content, ctaUrl }: { content: FourWeekSupportContent; ctaUrl?: string }) {
  return <section aria-labelledby="four-week-support-purchase" className="relative overflow-hidden rounded-[var(--radius-card)] bg-[linear-gradient(135deg,#2f6bff,#8b3cf9)] p-6 text-white shadow-[0_28px_70px_-25px_rgba(70,70,235,0.75)] sm:p-8 lg:p-10"><div className="pointer-events-none absolute -right-24 -top-24 h-64 w-64 rounded-full bg-white/20 blur-3xl" /><div className="relative"><h2 id="four-week-support-purchase" className="text-balance text-2xl font-black sm:text-3xl">{content.purchaseTitle}</h2><p className="mt-4 max-w-2xl text-lg leading-relaxed text-white/85">{content.purchaseText}</p><ul className="mt-7 grid gap-3 sm:grid-cols-2">{content.purchaseItems.map((item) => <li key={item} className="flex min-w-0 gap-3"><span aria-hidden="true" className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/20 text-xs font-black text-white">✓</span><span className="min-w-0 break-words leading-relaxed text-white/95">{item}</span></li>)}</ul><div className="mt-8 border-t border-white/20 pt-8 text-center"><p className="text-5xl font-black tracking-tight">{content.price}</p>{ctaUrl ? <><FourWeekSupportCta href={ctaUrl} tone="on-gradient" className="mt-6 w-full justify-center sm:w-auto sm:min-w-80" /><p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-white/80">{content.purchaseSupportText}</p></> : null}</div></div></section>;
}

function FourWeekEverydayLife({ content }: { content: FourWeekSupportContent }) {
  return <section aria-labelledby="four-week-everyday-life" className="rounded-2xl border border-[#ded4ff] bg-[linear-gradient(135deg,#f8f6ff,#edf4ff)] p-6 sm:p-8"><h2 id="four-week-everyday-life" className="text-balance text-2xl font-black sm:text-3xl">{content.everydayLifeTitle}</h2><div className="mt-5 space-y-4 text-lg leading-relaxed text-[var(--color-text-muted)]">{content.everydayLifeText.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></section>;
}

function FourWeekSupportFaq({ items }: { items: FourWeekSupportContent["faq"] }) {
  return <section aria-labelledby="four-week-support-faq" className="rounded-2xl border border-[var(--color-border-light)] bg-white p-6 shadow-[0_18px_45px_-35px_rgba(20,10,60,0.45)] sm:p-8"><h2 id="four-week-support-faq" className="text-2xl font-black sm:text-3xl">Nejčastější otázky</h2><dl className="mt-6 divide-y divide-[var(--color-border-light)]">{items.map((item) => <div key={item.question} className="py-5 first:pt-0 last:pb-0"><dt className="text-lg font-extrabold text-[var(--color-text)]">{item.question}</dt><dd className="mt-2 leading-relaxed text-[var(--color-text-muted)]">{item.answer}</dd></div>)}</dl></section>;
}

function FourWeekSupportFinalCta({ content, ctaUrl }: { content: FourWeekSupportContent; ctaUrl?: string }) {
  return <section aria-labelledby="four-week-support-final" className="rounded-[var(--radius-card)] border border-[#d7dcff] bg-[linear-gradient(135deg,#eef5ff,#f6efff)] p-6 text-center shadow-[0_18px_45px_-35px_rgba(20,10,60,0.45)] sm:p-8 lg:p-10"><h2 id="four-week-support-final" className="text-balance text-2xl font-black sm:text-3xl">{content.finalTitle}</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-[var(--color-text-muted)]">{content.finalText}</p><h3 className="mt-8 text-balance text-xl font-extrabold sm:text-2xl">{content.finalCtaTitle}</h3><p className="mt-3 text-lg font-bold text-[var(--color-accent-purple)]">{content.finalPriceText}</p>{ctaUrl ? <><FourWeekSupportCta href={ctaUrl} className="mt-6 w-full justify-center sm:w-auto sm:min-w-80" /><p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-[var(--color-text-muted)]">{content.purchaseSupportText}</p></> : null}</section>;
}

function FourWeekSupportCta({ href, tone = "brand", className = "" }: { href: string; tone?: PrivateCtaTone; className?: string }) {
  const external = href.startsWith("https://");
  return <PrivatePageCta href={href} tone={tone} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} className={className}>Chci 4týdenní podporu</PrivatePageCta>;
}

function PersonalDietReviewDetail({ page, content }: { page: PrivatePage; content: PersonalDietReviewContent }) {
  const ctaUrl = content.cta.active ? page.salesLinks[content.cta.salesLinkKey] : undefined;
  return <div className="overflow-hidden bg-white"><Container className="py-[var(--space-section)]"><div className="mx-auto max-w-4xl space-y-8"><ContentSection title={content.audienceTitle}><BulletList items={content.audience} /></ContentSection><ContentSection title={content.benefitsTitle}><p className="mb-6 text-lg leading-relaxed text-[var(--color-text-muted)]">{content.benefitsIntro}</p><DetailList items={content.benefits} /></ContentSection><ContentSection title={content.processTitle}><DetailList items={content.process} numbered /></ContentSection><PersonalDietReviewPurchase content={content} ctaUrl={ctaUrl} /><PersonalDietReviewFaq items={content.faq} /><PersonalDietReviewFinalCta content={content} ctaUrl={ctaUrl} />{content.additionalInfo ? <p className="rounded-2xl bg-[var(--color-surface-muted)] p-6 text-sm leading-relaxed text-[var(--color-text-muted)]">{content.additionalInfo}</p> : null}</div></Container></div>;
}

function PersonalDietReviewPurchase({ content, ctaUrl }: { content: PersonalDietReviewContent; ctaUrl?: string }) {
  return <section aria-labelledby="personal-diet-review-purchase" className="relative overflow-hidden rounded-[var(--radius-card)] bg-[linear-gradient(135deg,#10163a,#2a1b57)] p-6 text-white shadow-[var(--shadow-card)] sm:p-8 lg:p-10"><div className="pointer-events-none absolute -right-24 -top-24 h-64 w-64 rounded-full bg-[var(--color-accent-purple)]/35 blur-3xl" /><div className="relative"><h2 id="personal-diet-review-purchase" className="text-balance text-2xl font-black sm:text-3xl">{content.purchaseTitle}</h2><p className="mt-4 max-w-2xl text-lg leading-relaxed text-white/80">{content.purchaseText}</p><ul className="mt-7 grid gap-3 sm:grid-cols-2">{content.purchaseItems.map((item) => <li key={item} className="flex min-w-0 gap-3"><span aria-hidden="true" className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/15 text-xs font-black text-[#c4aeff]">✓</span><span className="min-w-0 break-words leading-relaxed text-white/90">{item}</span></li>)}</ul><div className="mt-8 border-t border-white/15 pt-8 text-center"><p className="text-5xl font-black tracking-tight">{content.price}</p>{ctaUrl ? <><PersonalDietReviewCta href={ctaUrl} tone="on-dark" className="mt-6 w-full justify-center sm:w-auto sm:min-w-80" /><p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-white/70">{content.purchaseSupportText}</p></> : null}</div></div></section>;
}

function PersonalDietReviewFaq({ items }: { items: PersonalDietReviewContent["faq"] }) {
  return <section aria-labelledby="personal-diet-review-faq" className="rounded-2xl border border-[var(--color-border-light)] bg-white p-6 shadow-[0_18px_45px_-35px_rgba(20,10,60,0.45)] sm:p-8"><h2 id="personal-diet-review-faq" className="text-2xl font-black sm:text-3xl">Nejčastější otázky</h2><dl className="mt-6 divide-y divide-[var(--color-border-light)]">{items.map((item) => <div key={item.question} className="py-5 first:pt-0 last:pb-0"><dt className="text-lg font-extrabold text-[var(--color-text)]">{item.question}</dt><dd className="mt-2 leading-relaxed text-[var(--color-text-muted)]">{item.answer}</dd></div>)}</dl></section>;
}

function PersonalDietReviewFinalCta({ content, ctaUrl }: { content: PersonalDietReviewContent; ctaUrl?: string }) {
  return <section aria-labelledby="personal-diet-review-final" className="rounded-[var(--radius-card)] border border-[#ded4ff] bg-[linear-gradient(135deg,#f8f6ff,#edf4ff)] p-6 text-center shadow-[0_18px_45px_-35px_rgba(20,10,60,0.45)] sm:p-8 lg:p-10"><h2 id="personal-diet-review-final" className="text-balance text-2xl font-black sm:text-3xl">{content.finalTitle}</h2><p className="mx-auto mt-4 max-w-2xl text-lg leading-relaxed text-[var(--color-text-muted)]">{content.finalText}</p><h3 className="mt-8 text-balance text-xl font-extrabold sm:text-2xl">{content.finalCtaTitle}</h3><p className="mt-3 text-lg font-bold text-[var(--color-accent-purple)]">{content.finalPriceText}</p>{ctaUrl ? <><PersonalDietReviewCta href={ctaUrl} className="mt-6 w-full justify-center sm:w-auto sm:min-w-80" /><p className="mx-auto mt-4 max-w-xl text-sm leading-relaxed text-[var(--color-text-muted)]">{content.purchaseSupportText}</p></> : null}</section>;
}

function PersonalDietReviewCta({ href, tone = "brand", className = "" }: { href: string; tone?: PrivateCtaTone; className?: string }) {
  const external = href.startsWith("https://");
  return <PrivatePageCta href={href} tone={tone} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} className={className}>Chci svůj osobní rozbor</PrivatePageCta>;
}
